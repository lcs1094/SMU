...
void printMok(char mok[20][20][4]);
int check(char mok[20][20][4]);
...
int check(char mok[20][20][4]){	// 1 : o승 2 : x승 3: 무승부
	int h_o_count = 0;	// 가로
	int h_x_count = 0;
	int v_o_count = 0;	// 세로
	int v_x_count = 0;
				...
				if(i<15 && j<15){
					...
					for(ii=1;ii<5;ii++){
						...
						if(strcmp(mok[o_checkpoint_v+ii][o_checkpoint_h] , "o") == 0)
								v_o_count++;
						for(jj=1;jj<5;jj++){
							if(strcmp(mok[o_checkpoint_v][o_checkpoint_h+jj] , "o") == 0)
								h_o_count++;
							if(ii == jj && strcmp(mok[o_checkpoint_v+ii][o_checkpoint_h+jj] , "o") == 0)
								d_o_count++;
							if(ii == jj && strcmp(mok[o_checkpoint_v+ii][o_checkpoint_h-jj] , "o") == 0)
								d2_o_count++;
						}
				...
				if(i<15 && j<15){
					...
					for(ii=1;ii<5;ii++){
						...
						if(strcmp(mok[x_checkpoint_v+ii][x_checkpoint_h] , "x") == 0)
								v_x_count++;
						for(jj=1;jj<5;jj++){
							if(strcmp(mok[x_checkpoint_v][x_checkpoint_h+jj] , "x") == 0)
								h_x_count++;
							if(ii == jj && strcmp(mok[x_checkpoint_v+ii][x_checkpoint_h+jj] , "x") == 0)
								d_x_count++;
							if(ii == jj && strcmp(mok[x_checkpoint_v+ii][x_checkpoint_h-jj] , "x") == 0)
								d2_x_count++;
						}
...
int main(void){
	...
	do{
		...
		if(strcmp(mok[x][y], "o") == 0 || strcmp(mok[x][y] , "x") == 0 ){
			printf("돌이 이미 있습니다.\n");
			continue;
		}
		if(turn){
